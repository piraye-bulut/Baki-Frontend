...
export const Matla = () => {
  const { id } = useParams();
  const matla = data.matlalar[parseInt(id) - 1];

  if (!matla) {
    return <p>Matla bulunamadı</p>;
  }
  ...
  return (
    <div className="min-h-screen bg-orange-100 py-20 flex flex-col items-center">
      <div className="bg-orange-200 p-6 rounded-lg shadow-lg w-11/12 md:w-3/4 lg:w-1/2 text-center border border-gray-300 mb-4">
        <h2 className="text-2xl font-bold text-red-700 my-4">{matla.başlık}</h2>
        <div className="text-left">
          {matla.dizeler.map((dize, index) => (
            <p key={index}>{dize}</p>
          ))}
        </div>
      ...
      <div className="bg-orange-200 p-6 rounded-lg shadow-lg w-11/12 md:w-3/4 lg:w-1/2 text-center border border-gray-300">
        <CommentSection poemId={id} poemType="matla" /> {/* Yorum bileşenini kullanın */}
      </div>
    </div>
  );
};

export default Matla;